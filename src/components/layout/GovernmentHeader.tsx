...
export function GovernmentHeader() {
  ...
  return (
    <header className="w-full bg-white border-b border-[#DCE3EA] shadow-sm sticky top-0 z-50">
      ...
      <div className="bg-[#0B2945] text-slate-300 text-xs py-1 px-4 sm:px-8 flex justify-between items-center border-b border-[#123F68]">
        <div className="flex items-center space-x-3">
          ...
          <span className="hidden md:inline text-slate-200">Ministry of Statistics and Programme Implementation (MoSPI)</span>
        </div>
        <div className="flex items-center space-x-4 text-[11px]">
          <button 
            onClick={() => setIsLangHindi(!isLangHindi)}
            className="hover:text-white flex items-center font-semibold bg-[#123F68] px-2.5 py-0.5 rounded border border-slate-600 transition-colors"
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-4 bg-white">
        ...
        <div className="flex items-center space-x-3 sm:space-x-4 shrink-0">
          ...
          <div className="flex flex-col cursor-pointer pr-3" onClick={() => navigate('/dashboard')}>
            <span className="text-sm sm:text-base font-bold text-[#172B3A] tracking-tight leading-snug font-display">
              {isLangHindi ? 'भारत सरकार' : 'Government Of India'}
            </span>
            <span className="text-sm sm:text-base font-bold text-[#172B3A] leading-snug font-display">
          ...
          <div className="hidden xl:flex items-center space-x-4 pl-4 border-l border-slate-200 shrink-0">
            <img 
            ...
            />
            <div className="h-10 border-r border-slate-300"></div>
            <img 
        ...
        <div className="flex items-center space-x-3 shrink-0">
          ...
          <div className="relative hidden md:block w-48 lg:w-60 xl:w-64" ref={searchRef}>
            <span className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
              <Search className="w-4 h-4 text-[#526477]" />
            </span>
            <input
              type="text"
              value={search}
              ...
              className="w-full pl-9 pr-3 py-1.5 border border-[#DCE3EA] rounded bg-[#F5F7FA] text-xs text-[#172B3A] font-medium focus:outline-none focus:ring-2 focus:ring-[#123F68] focus:bg-white transition-all shadow-inner"
              placeholder="Search member, state, constituency..."
            />
            {isSearchFocused && search.length > 2 && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded shadow-xl border border-[#DCE3EA] text-[#172B3A] max-h-80 overflow-y-auto z-50">
                {searchResults.length === 0 ? (
                  <div className="p-3 text-xs text-[#526477] text-center">No matching records found.</div>
                ) : (
                  <div className="py-1">
                    {searchResults.map(result => (
                      <div 
                        ...
                        className="px-3 py-2 hover:bg-[#EAF3FA] cursor-pointer border-b border-slate-100 last:border-0"
                      >
                        <div className="font-bold text-xs text-[#0B2945]">{result.name}</div>
                        <div className="text-[10px] text-[#526477] flex gap-2 mt-0.5">
                          <span className="bg-[#123F68] text-white px-1 py-0.2 rounded text-[9px] uppercase font-bold">{result.house}</span>
          ...
          {/* House Selector */}
          <div className="hidden lg:flex items-center space-x-1.5 bg-[#F5F7FA] px-2.5 py-1 rounded border border-[#DCE3EA] shrink-0">
            <span className="text-[10px] font-bold text-[#526477] uppercase tracking-wider">House:</span>
            <select 
              className="text-xs bg-transparent font-bold text-[#0B2945] focus:outline-none cursor-pointer"
              value={house}
          ...
          {/* Role Selector */}
          <div className="hidden sm:flex items-center space-x-1.5 bg-[#EAF3FA] px-2.5 py-1 rounded border border-[#B0C4DE] shrink-0">
            <span className="text-[10px] font-bold text-[#123F68] uppercase tracking-wider">Authority:</span>
            <select 
              className="text-xs bg-transparent font-extrabold text-[#0B2945] focus:outline-none cursor-pointer"
              value={role}
          ...
          <div className="relative" ref={notificationsRef}>
            <button 
              ...
              className="relative p-2 text-[#123F68] hover:bg-[#EAF3FA] rounded transition-colors shrink-0" 
              title="System Alerts"
            >
            ...
            {isNotificationsOpen && (
              <div className="absolute top-full right-0 mt-1 w-80 bg-white rounded-md shadow-xl border border-[#DCE3EA] z-50 overflow-hidden">
                <div className="bg-[#0B2945] text-white px-4 py-2 font-bold text-xs flex justify-between items-center">
                  <span>System Alerts</span>
                  <span className="bg-red-500 text-white px-2 py-0.5 rounded-full text-[9px]">{activeAlerts.length} New</span>
                </div>
                <div className="max-h-80 overflow-y-auto">
                  {activeAlerts.length === 0 ? (
                    <div className="p-4 text-center text-xs text-slate-500">No critical alerts at this time.</div>
                  ) : (
                      ...
                      return (
                        <div key={idx} className="p-3 border-b border-slate-100 hover:bg-slate-50 cursor-pointer" onClick={() => { setIsNotificationsOpen(false); navigate('/dashboard/alerts'); }}>
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <div className="text-xs font-bold text-slate-900 leading-tight">Critical Risk: {member?.name || alert.member_id}</div>
                              <div className="text-[10px] text-slate-500 mt-1">{alert.primary_signal}</div>
          ...
          {/* User Badge */}
          <div className="flex items-center space-x-2 pl-2 border-l border-[#DCE3EA] shrink-0">
            <div className="w-8 h-8 rounded bg-[#123F68] text-white flex items-center justify-center font-bold text-xs border border-[#0B2945] shadow-xs">
              GOI
            </div>
          ...
          <button 
            ...
            className="lg:hidden p-2 text-[#0B2945] hover:bg-slate-100 rounded shrink-0"
          >
      ...
      <nav className="bg-[#123F68] text-white shadow-md">
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden bg-[#0B2945] px-4 pt-2 pb-4 space-y-1 text-sm border-t border-[#123F68]">
            <div className="p-2 bg-[#123F68] rounded mb-2 flex items-center justify-between">
              <span className="text-xs font-bold text-amber-400 uppercase">Role: {role}</span>
              <select 
                className="text-xs bg-[#0B2945] text-white p-1 rounded border border-slate-600"
                value={role}
                onChange={(e) => setRole(e.target.value as RoleSelection)}
              >
                {roles.map(r => (
