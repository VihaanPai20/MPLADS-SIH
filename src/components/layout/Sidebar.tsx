...
export function Sidebar() {
  return (
    <aside className="w-64 bg-forest-deep text-mutedText flex flex-col h-full shrink-0">
      <div className="h-16 flex items-center px-6 font-bold text-white tracking-wider border-b border-forest-primary">
        MPLADS INTELLIGENCE
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-4 scrollbar-thin scrollbar-thumb-slate-700">
        {navSections.map((section) => (
          <div key={section.title} className="mb-6">
            <h3 className="px-6 text-xs font-semibold text-mutedText uppercase tracking-wider mb-2">
              {section.title}
            </h3>
                  ...
                  <li key={item.name}>
                    <NavLink
                      ...
                      className={({ isActive }) => cn(
                        ...
                        isActive 
                          ? 'text-white bg-forest-primary' 
                          : 'hover:text-white hover:bg-forest-deep'
                      )}
                    >
      ...
      <div className="mt-auto border-t border-forest-primary p-4">
        <NavLink
          to="/settings"
          className={({ isActive }) => cn(
