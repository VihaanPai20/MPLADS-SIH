...
export function Dashboard() {
  ...
  if (loading) return <div className="p-8 text-mutedText">Loading dashboard...</div>;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            {role === 'Administrator' && <UserCog className="w-5 h-5 text-forest-secondary" />}
            {role === 'Ministry' && <Landmark className="w-5 h-5 text-forest-primary" />}
            {role === 'State Nodal Authority' && <Building2 className="w-5 h-5 text-forest-primary" />}
            ...
            <span className="text-sm font-bold uppercase tracking-wider text-mutedText">{role} View</span>
          </div>
          <h1 className="text-2xl font-bold text-charcoal">National MPLADS Overview</h1>
        ...
        {(role === 'Administrator' || role === 'Ministry') && quality && (
          <div className="bg-forest-deep text-white rounded-lg p-4 text-xs shadow-md border border-forest-primary w-full sm:w-64 flex flex-col gap-2">
            <div className="flex items-center justify-between border-b border-forest-primary pb-2">
              <span className="font-bold tracking-wider text-white">DATA QUALITY</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-indigo-50 p-6 rounded-lg border border-indigo-200 shadow-sm">
          ...
          <div className="text-3xl font-bold text-indigo-950">{stats.totalWorks.toLocaleString()}</div>
        </div>
        ...
        <div className="bg-emerald-50 p-6 rounded-lg border border-emerald-200 shadow-sm">
          ...
          <div className="text-3xl font-bold text-emerald-950">{formatCurrency(stats.totalSanctioned)}</div>
        </div>

        <div className="bg-violet-50 p-6 rounded-lg border border-violet-200 shadow-sm relative group">
          <div className="absolute top-2 right-2 px-2 py-0.5 bg-violet-200 text-violet-800 text-[10px] font-bold rounded-sm uppercase">Estimated</div>
          ...
          <div className="text-3xl font-bold text-violet-950">{formatCurrency(stats.totalSanctioned * 0.72)}</div>
        </div>
        ...
        <div className="bg-amber-50 p-6 rounded-lg border border-amber-200 shadow-sm relative group">
          ...
          <div className="text-3xl font-bold text-amber-950">{stats.complianceExceptions}</div>
        </div>

        <div className="bg-cyan-50 p-6 rounded-lg border border-cyan-200 shadow-sm relative group">
          <div className="flex items-center text-cyan-700 mb-2">
            <AlertTriangle className="w-4 h-4 mr-2 text-cyan-600" />
            <span className="text-sm font-medium uppercase tracking-wider">Potential Duplicates</span>
          </div>
          <div className="text-3xl font-bold text-cyan-950">{stats.potentialDuplicates}</div>
        </div>
      </div>
    </div>
  );
