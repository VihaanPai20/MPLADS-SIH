...
export function FinancialAnalytics() {
  ...
  if (financialLoading || mlLoading) return <div className="p-8 text-mutedText">Loading financial analytics...</div>;
  if (!analytics) return null;

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-charcoal">Financial Analytics</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-6 rounded-lg border border-brandBorder shadow-sm">
          <div className="text-sm font-semibold text-mutedText uppercase tracking-wider mb-2">Total Allocated (Actual)</div>
      ...
      <div className="bg-white p-6 rounded-lg border border-brandBorder shadow-sm">
        <h3 className="text-sm font-bold text-charcoal uppercase mb-4">Total Allocation by State (₹ Crores) - Actuals</h3>
        <div className="h-96">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={analytics.stateData.slice(0, 15)} layout="vertical" margin={{ top: 0, right: 30, left: 100, bottom: 0 }}>
              ...
              <Bar dataKey="amount" fill="#356B52" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
